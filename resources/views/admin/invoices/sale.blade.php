...
<head>
    <meta charset="utf-8">
    <title>Sales Invoice {{ $sale->sale_id }}</title>
    <style>
...
<body>
    ...
    <table class="meta">
        <tr>
            <td>
                ...
                <strong>Date:</strong> {{ $sale->sale_date->format('Y-m-d') }}
            </td>
            <td style="text-align: right;">
                <strong>Customer:</strong> {{ $sale->customer?->name ?? $sale->customer_name ?? 'Walk-in Customer' }}<br>
    ...
    <table class="summary">
        ...
        <tr><td>Discount</td><td>{{ number_format($sale->discount_amount, 2) }}</td></tr>
        <tr><td>VAT</td><td>{{ number_format($sale->vat_amount, 2) }}</td></tr>
        <tr class="grand-total"><td>Grand Total</td><td>{{ number_format($sale->total_amount, 2) }}</td></tr>
        <tr><td>Paid</td><td>{{ number_format($sale->paid_amount, 2) }}</td></tr>
        <tr><td>Due</td><td>{{ number_format($sale->due_amount, 2) }}</td></tr>
    </table>
</body>
</html>
